		...
		<?php if ( ! get_post_meta( $post_id, 'disable_title', true ) || ! get_post_meta( $post_id, 'disable_breadcrumbs', true ) ): ?>
			<div class="container">
				...
				if ( ! get_post_meta( $post_id, 'disable_breadcrumbs', true ) ) {
					crypterio_breadcrumbs();
				}
				?>
				<?php if ( ! get_post_meta( $post_id, 'disable_title', true ) ): ?>
					<?php if( crypterio_page_title( false, esc_html__( 'News', 'crypterio' ), esc_html__( 'Careers', 'crypterio' ) ) ): ?>
						<h1 class="h2"><?php echo crypterio_page_title( false, esc_html__( 'News', 'crypterio' ), esc_html__( 'Careers', 'crypterio' ) ); ?></h1>
					<?php endif; ?>
				<?php endif; ?>
			</div>
		<?php endif; ?>
