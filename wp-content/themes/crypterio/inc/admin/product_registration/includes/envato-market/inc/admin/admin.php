...
if ( ! class_exists( 'Envato_Market_Admin' ) && class_exists( 'Envato_Market' ) ) :
	...
	class Envato_Market_Admin {
		...
		public function __clone() {
			_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'blockchain' ), '1.0.0' );
		}
		...
		public function maybe_delete_transients() {
			if ( isset( $_POST[ envato_market()->get_option_name() ] ) ) {
				...
				if ( isset( $_POST['_wpnonce'] ) && ! wp_verify_nonce( $_POST['_wpnonce'], envato_market()->get_slug() . '-options' ) ) {
		 			wp_die( esc_html__( 'You do not have sufficient permissions to delete transients.', 'blockchain' ) );
				}
		...
		public function admin_enqueue_script() {
			...
			$exports = array(
				...
				'i18n' => array(
					'save' => esc_html__( 'Save', 'blockchain' ),
					'remove' => esc_html__( 'Remove', 'blockchain' ),
					'cancel' => esc_html__( 'Cancel', 'blockchain' ),
					'error' => esc_html__( 'An unknown error occurred. Try again.', 'blockchain' ),
				),
			);
		...
		public function render_templates() {
			...
			<script type="text/html" id="tmpl-envato-market-auth-check-button">
				<a href="<?php echo esc_url( add_query_arg( array( 'authorization' => 'check' ), envato_market()->get_page_url() ) ); ?>" class="button button-secondary auth-check-button" style="margin:0 5px"><?php esc_html_e( 'Test API Connection', 'blockchain' ); ?></a>
			</script>

			<script type="text/html" id="tmpl-envato-market-item">
				<li data-id="{{ data.id }}">
					<span class="item-name"><?php esc_html_e( 'ID', 'blockchain' ); ?>: {{ data.id }} - {{ data.name }}</span>
					<button class="item-delete dashicons dashicons-dismiss">
						<span class="screen-reader-text"><?php esc_html_e( 'Delete', 'blockchain' ) ?></span>
					</button>
			...
			<script type="text/html" id="tmpl-envato-market-dialog-remove">
				<div id="envato-market-dialog-remove" title="<?php esc_html_e( 'Remove Item', 'blockchain' ) ?>">
					<p><?php esc_html_e( 'You are about to remove the connection between the Envato Market API and this item. You cannot undo this action.', 'blockchain' ) ?></p>
				</div>
			</script>

			<script type="text/html" id="tmpl-envato-market-dialog-form">
				<div id="envato-market-dialog-form" title="<?php esc_html_e( 'Add Item', 'blockchain' ) ?>">
					<form>
						<fieldset>
							<label for="token"><?php esc_html_e( 'Token', 'blockchain' ) ?></label>
							<input type="text" name="token" class="widefat" value="" />
							<p class="description"><?php esc_html_e( 'Enter the Envato API Personal Token.', 'blockchain' ); ?></p>
							<label for="id"><?php esc_html_e( 'Item ID', 'blockchain' ) ?></label>
							<input type="text" name="id" class="widefat" value="" />
							<p class="description"><?php esc_html_e( 'Enter the Envato Item ID.', 'blockchain' ); ?></p>
						...
						<div class="envato-card-top">
							...
							<div class="column-name">
								<h4>
									...
									<span class="version" aria-label="<?php esc_attr_e( 'Version %s', 'blockchain' ); ?>"><?php esc_html_e( 'Version', 'blockchain' ); ?> {{ data.version }}</span>
								</h4>
							</div>
							<div class="column-description">
								...
								<p class="author">
									<cite><?php esc_html_e( 'By', 'blockchain' ); ?> {{ data.author }}</cite>
								</p>
							</div>
						</div>
						<div class="envato-card-bottom">
							<div class="column-actions">
								<a href="{{{ data.install }}}" class="button button-primary">
									<span aria-hidden="true"><?php esc_html_e( 'Install', 'blockchain' ); ?></span>
									<span class="screen-reader-text"><?php esc_html_e( 'Install', 'blockchain' ); ?> {{ data.name }}</span>
								</a>
							</div>
						</div>
					</div>
		...
		public function register_settings() {
			...
			add_settings_section(
			 ...
			 esc_html__( 'Global OAuth Personal Token', 'blockchain' ),
			...
			add_settings_section(
			 ...
			 esc_html__( 'Single Use OAuth Personal Tokens', 'blockchain' ),
			...
			add_settings_field(
				'items',
			 esc_html__( 'Envato Market Items', 'blockchain' ),
		...
		public function install_plugin( $plugin ) {
			if ( ! current_user_can( 'install_plugins' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . esc_html__( 'Installing Plugin...', 'blockchain' ) . '</h1>
					<p>' . esc_html__( 'You do not have sufficient permissions to install plugins on this site.', 'blockchain' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . esc_html__( 'Return to Plugin Installer', 'blockchain' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . esc_html__( 'Installing Plugin...', 'blockchain' ) . '</h1>
					<p>' . esc_html__( 'An error occurred, please check that the item ID is correct.', 'blockchain' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=plugins' ) . '">' . esc_html__( 'Return to Plugin Installer', 'blockchain' ) . '</a>
				</div>';
				wp_die( $msg );
			}

			$title = sprintf( esc_html__( 'Installing Plugin: %s', 'blockchain' ), $api->name . ' ' . $api->version );
		...
		public function install_theme( $theme ) {
			if ( ! current_user_can( 'install_themes' ) ) {
				$msg = '
				<div class="wrap">
					<h1>' . esc_html__( 'Installing Theme...', 'blockchain' ) . '</h1>
					<p>' . esc_html__( 'You do not have sufficient permissions to install themes on this site.', 'blockchain' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . esc_html__( 'Return to Theme Installer', 'blockchain' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			if ( empty( $array_api ) ) {
				...
				<div class="wrap">
					<h1>' . esc_html__( 'Installing Theme...', 'blockchain' ) . '</h1>
					<p>' . esc_html__( 'An error occurred, please check that the item ID is correct.', 'blockchain' ) . '</p>
					<a href="' . esc_url( 'admin.php?page=' . envato_market()->get_slug() . '&tab=themes' ) . '">' . esc_html__( 'Return to Plugin Installer', 'blockchain' ) . '</a>
				</div>';
				wp_die( $msg );
			}
			...
			$title = sprintf( esc_html__( 'Installing Theme: %s', 'blockchain' ), $api->name . ' ' . $api->version );
		...
		public function ajax_upgrade_theme() {
			...
			if ( $theme_data->exists() && $theme_data->get( 'Version' ) ) {
				$status['oldVersion'] = sprintf( esc_html__( 'Version %s', 'blockchain' ), $theme_data->get( 'Version' ) );
			}

			if ( ! current_user_can( 'update_themes' ) ) {
				$status['error'] = esc_html__( 'You do not have sufficient permissions to update themes for this site.', 'blockchain' );
			...
			if ( is_array( $result ) && ! empty( $result[ $theme ] ) ) {
				...
				if ( $theme_data->exists() && $theme_data->get( 'Version' ) ) {
					$status['newVersion'] = sprintf( esc_html__( 'Version %s', 'blockchain' ), $theme_data->get( 'Version' ) );
				}
				...
				$status['error'] = esc_html__( 'Unable to connect to the filesystem. Please confirm your credentials.', 'blockchain' );
		...
		public function ajax_add_item() {
			if ( ! check_ajax_referer( self::AJAX_ACTION, 'nonce', false ) ) {
				...
				wp_send_json_error( array( 'message' => esc_html__( 'The Token is missing.', 'blockchain' ) ) );
			} elseif ( empty( $_POST['id'] ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'The Item ID is missing.', 'blockchain' ) ) );
			}
			...
			if ( false === $request ) {
				wp_send_json_error( array( 'message' => esc_html__( 'The Token or Item ID is incorrect.', 'blockchain' ) ) );
			}

			if ( false === envato_market()->api()->download( $_POST['id'], $args ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'The item cannot be downloaded.', 'blockchain' ) ) );
			}
			...
			wp_send_json_error( array( 'message' => esc_html__( 'An unknown error occurred.', 'blockchain' ) ) );
		}
