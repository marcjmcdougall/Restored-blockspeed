...
if ( ! class_exists( 'Envato_Market_API' ) && class_exists( 'Envato_Market' ) ) :
	...
	class Envato_Market_API {
		...
		public function request( $url, $args = array() ) {
			...
			if ( empty( $token ) ) {
				return new WP_Error( 'api_token_error', esc_html__( 'An API token is required.', 'blockchain' ) );
			}
			...
			if ( 200 !== $response_code && ! empty( $response_message ) ) {
				...
				return new WP_Error( $response_code, esc_html__( 'An unknown API error occurred.', 'blockchain' ) );
			} else {
				$return = json_decode( wp_remote_retrieve_body( $response ), true );
				if ( null === $return ) {
					return new WP_Error( 'api_error', esc_html__( 'An unknown API error occurred.', 'blockchain' ) );
				}
				return $return;
			}
		}
