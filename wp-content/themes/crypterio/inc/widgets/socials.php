...
class Stm_Socials_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo crypterio_sanitize_text_field($args['before_widget']);
		if ( ! empty( $title ) ) {
			echo crypterio_sanitize_text_field($args['before_title'] . esc_html( $title ) . $args['after_title']);
		}
		...
		echo crypterio_sanitize_text_field($args['after_widget']);
	}
	...
	public function form( $instance ) {
		...
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Social Network', 'crypterio' );
		}
