...
if( !empty( $vc_image_size ) ) {
	...
	$image_size = 'crypterio-image-350x204-croped';
}

if( $style == 'style_3' ){
	$image_size = 'crypterio-image-350x250-croped';
}
...
<div class="info_box<?php echo esc_attr( $css_class ); ?> <?php echo esc_attr($style_l15); ?>">
	...
	<?php if( $style == 'style_6' ) : ?>
		...
		<div class="info_box_text">
			<div class="title">
				<div class="icon">
					<i class="<?php echo esc_attr( $title_icon ); ?>"<?php echo crypterio_sanitize_text_field($icon_style); ?>></i>
				</div>
			...
			if ( $link['url'] ) {
				if ( ! $link['title'] ) {
					$link['title'] = esc_html__( 'Read more', 'crypterio' );
				}
				if ( ! $link['target'] ) {
					$link['target'] = '_self';
				}
	...
	<?php else : ?>
		...
		<?php if( $style == 'style_4' ) : ?>
			<?php if( $image ): ?>
				...
				?>
				<div class="info_box_image"><?php echo crypterio_sanitize_text_field($image_data['thumbnail']); ?></div>
			<?php endif; ?>
		<?php else : ?>
			<?php if( empty( $vc_image_size ) && $image && $thumbnail = wp_get_attachment_image( $image, $image_size ) ){ ?>
				<div class="info_box_image"><?php echo crypterio_sanitize_text_field($thumbnail); ?></div>
			<?php } elseif( $image && !empty( $vc_image_size ) ) { ?>
				...
				?>
				<div class="info_box_image"><?php echo crypterio_sanitize_text_field($vc_image_data['thumbnail']); ?></div>
			<?php } ?>
		<?php endif; ?>
